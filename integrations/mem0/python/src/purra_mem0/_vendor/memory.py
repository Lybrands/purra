# Derived from mem0ai 2.0.19 (Apache-2.0). PurrA adds constructor provider injection.
...
class Memory(MemoryBase):
    def __init__(self, config: MemoryConfig = MemoryConfig(), *, llm=None, embedder=None):
        ...
        self.provider_binding = {"llm": "injected" if llm is not None else "native", "embedder": "injected" if embedder is not None else "native"}
        try:
            self.embedding_model = embedder if embedder is not None else EmbedderFactory.create(
            ...
            self.llm = llm if llm is not None else LlmFactory.create(self.config.llm.provider, self.config.llm.config)
