...
`propose_memory_relations(memory, refs, relations=allowed_names, extract=host_extract)`
is exported from `purra_mem0`. Its async extractor receives `(input, signal)` and
