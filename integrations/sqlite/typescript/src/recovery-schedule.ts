...
export class SqliteRecoverySchedule {
  ...
  private row(extra: Extra, runId: string): { revision: number; failures: number; notBeforeMs: number } {
    ...
    const row = Object.hasOwn(rows, runId) ? rows[runId] : { revision: 0, failures: 0, notBeforeMs: 0 };
  ...
  private save(extra: Extra, runId: string, row: object): void {
  ...
  async ready(runId: string): Promise<number | null> {
    return this.transaction(async extra => {
      const row = this.row(extra, runId);
      return row.notBeforeMs <= integer((this.options.clockMs ?? Date.now)()) ? row.revision : null;
    }, true);
  }
  async wake(runId: string): Promise<number> {
    return this.transaction(async extra => {
      const revision = integer(this.row(extra, runId).revision + 1);
      this.save(extra, runId, { revision, failures: 0, notBeforeMs: 0 });
      return revision;
    }, false);
  }
  async settle(runId: string, revision: number, failed: boolean): Promise<boolean> {
    ...
    return this.transaction(async extra => {
      ...
      this.save(extra, runId, { revision: integer(revision + 1), failures,
