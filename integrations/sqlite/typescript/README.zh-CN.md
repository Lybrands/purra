...
`purra` 导出 `StorageSession`，统一仓储状态版本与独立日志的恢复。
`storage.transaction(async (stores, extra) => ...)` 提供事务内端口与扩展元数据。
会话及延迟日志回调不能离开事务后继续使用；仅保存快照正文不是完整备份。
对外仓储方法由显式端口列表声明。
...
已有 v4 数据库在打开时补建索引，需要耗时和额外磁盘，新增事件也需要维护索引。
元数据仍按作用域保存，
