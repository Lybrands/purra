...
# Additive media capability: absent in older text-only checkpoints. Omit unknown
# when writing so their canonical encoding and identity remain unchanged.
_OPTIONAL_FIELDS = {"ModelProtocolCapabilities": {"image_input": "unknown"}}
...
def _decode(row, depth=0):
    ...
    if kind == "record":
        ...
        if not isinstance(row[2], dict) or set(row[2]) - set(names) or set(names) - set(row[2]) - set(optional):
            raise ValueError("invalid storage record fields")
        return cls(**{key: _decode(row[2][key], depth + 1) for key in row[2]})
    if kind == "map":
