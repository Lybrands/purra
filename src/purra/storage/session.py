...
STORAGE_STATE_SCHEMA = "purra.storage-state/python/v2"
...
class StorageSession:
    def __init__(self, body: str | None = None):
        ...
        if body is not None:
            ...
            self.state = AdapterState.from_groups(saved["groups"])
            ...
            self._validate_runs()
            validate_stored_dependencies(self.state.tree.runs)
        self._adapters = InMemoryAgentAdapters(state=self.state)
        self._prior_sequences = dict(self._state.sequences)
        self._prior_runs = set(self._state.runs)

    @property
    def runs(self):
        return self._adapters.runs
    ...
    @property
    def run_tree(self):
        return self._adapters.run_tree

    @property
    def artifacts(self):
        return self._adapters.artifacts

    @property
    def artifact_claims(self):
        return self._adapters.artifact_claims

    @property
    def artifact_maintenance(self):
        return self._adapters.artifact_maintenance

    @property
    def long_tasks(self):
        return self._adapters.long_tasks
    ...
    def _validate_runs(self):
        ...
        for run_id, record in self._state.runs.items():
            if not isinstance(run_id, str) or not isinstance(record, StoredRun) or not isinstance(record.params, RunCreateParams) or not isinstance(record.status, RunStatus):
        ...
        if any(not isinstance(v, StoredStream) or v.spec.run_id not in self._state.runs for v in self._state.streams.values()):
            raise ValueError("invalid stored stream")

    def export_snapshot(self) -> str:
        groups = self.state.to_groups()
    ...
    def find_tree_run(self, run_id):
        return self.state.tree.runs.get(run_id)
    ...
    def has_unsettled_execution(self):
        ...
        return bool(self.running_run_ids() or self.claims or any(
            ...
            for run in self.state.tree.runs.values()
        ))
...
STORAGE_PORT_METHODS = {
    ...
    "run_tree": frozenset(('aggregate_runs', 'begin_root', 'cancel_subtree', 'claim_run', 'close_agent', 'complete_run', 'continue_agent', 'fail_run', 'get_agent', 'get_checkpoint', 'get_run', 'list_descendants', 'list_agent_descendants', 'list_runnable', 'mark_waiting', 'release_waiting', 'renew_run_lease', 'require_run_claim', 'spawn_agents', 'suspend_run')),
