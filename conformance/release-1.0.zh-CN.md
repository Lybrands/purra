...
当前交付面向本地部署、由宿主嵌入的框架，宿主可以调用远端模型 API。跨机器 worker、远程 Agent 会话、新增音频／视频／实时能力及尚无具体对象的记忆迁移均延期，不作为候选发布前置。不新增数据库识别或第二数据库后端。

宿主策略在 Run 前选择模型；下一轮对话可以携带宿主选定的历史，以另一模型绑定提交新的 Run。执行中的 Run 保持原绑定。可选 Mem0 提供[有序记忆选择](../integrations/mem0/selection.md)和[只读关系候选](../integrations/mem0/relations.md)，宿主负责检索排序、抽取、关系语义和显式写入链接。Core 与组件继续检查授权、预算、取消和恢复一致性。参见[宿主策略边界](../docs/host-policies.md)。
...
可选 SQLite 适配器默认使用 v4，显式激活持久审批后支持 v5，拒绝其他格式版本；Python 与 TypeScript 快照不能互换。宿主自有持久化 schema 不属于这些适配器格式，激活流程、规划预览策略及宿主存储检查见[升级指南](../docs/migrations/1.0.zh-CN.md)。新的 invocation receipt 为 schema v3，TypeScript 状态导入拒绝不支持的回执版本。因此，适配器格式标记相同不等于 0.x 历史或活跃 checkpoint 一定兼容。本候选没有通用 0.x 历史迁移工具；选择新数据库也不授权删除旧数据。恢复诊断不授予 resume 权限。
