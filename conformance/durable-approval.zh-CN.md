...
支持边界及外部验收状态见[1.0 接入交接](runtime-extension-acceptance.zh-CN.md)。
...
未知效果、回执落盘失败和已提交回执重放。Child 写审批不受支持；真实 Provider 仅有[已记录组合](runtime-extension-acceptance.zh-CN.md#已验证的真实-provider-组合)的证据，业务 MCP 和下游验收仍未通过。不能据此宣称 1.0 已验收。
...
双端新增独立写发现入口，详见 MCP 包 README。写绑定必须具备宿主声明的 confirm
策略、匹配的 write/destructive 效果、非空 scope 和绑定/资源身份；Core 强制要求持久
审批网关及其同一回执存储。原只读入口与 schema 1 快照保留。写快照为 schema 2，
包含宿主身份；注册的有效 bindingRevision 使用完整目录摘要。宿主构造续点意图时
必须使用当前注册身份，SQLite 在派发及回执重放前再次比对。

共享结果案例及本地官方 SDK 服务覆盖成功 committed、请求前拒绝 not_started、
...
双端安装消费者通过 stdio 访问单独启动的官方 Python MCP SDK 服务，报告记录实际协商
协议。服务仅拥有新建临时目录，固定值文件及独立事件账本均 fsync，不使用业务资源。

| 服务场景 | 远端实际写入次数 | 宿主效果／回执 |
| --- | ---: | --- |
| 正常成功 | 1 | committed 回执 |
| 写入后抑制响应，模拟丢失 | 1 | 保留 unknown claim |
| 写入前退出 | 0 | 保留 unknown claim |
| 写入后退出 | 1 | 保留 unknown claim |
| 写入后返回错误 | 1 | 保留 unknown claim |

每个场景均验证：SQLite 重新打开后仍等待审批且没有额外模型调用或远端派发；宿主批准
之后才执行；结束后再次恢复明确由 run_terminal 拒绝，而非依赖 MCP 断连阻止执行。
远端账本中只有一次请求，关闭 transport 后确认每个服务 PID 都已退出。账本只供验证，
不会自动触发对账。尤其“写入前退出确实未写入”来自独立文件证据，不能仅由传输错误推断。

复现命令见 MCP README 和 CI 安装消费者步骤。模型仍为脚本桩，服务为受控合成服务；
不计作真实 Provider、第三方业务 MCP 或下游启用验收，也不代表远程 CI 已运行。
